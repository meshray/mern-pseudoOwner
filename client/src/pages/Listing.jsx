        ...
        <div>
          <Slider {...sliderSettings}>
            {listing.imageUrls.map((url, index) => (
              <div key={index}>
                <img src={url} alt={`slide-${index}`} className='h-[550px] w-full object-cover' />
              </div>
            ))}
          </Slider>
         ...
         <div className='flex flex-col max-w-4xl mx-auto p-3 my-7 gap-2'>
            ...
            <div className='flex gap-4'>
              <p className='bg-red-900 w-full max-w-[200px] text-white text-center p-1 rounded-md'>
                {listing.type === 'intercity' ? 'Allowed intercity' : 'Not Allowed intercity'}
              </p>
              {listing.offer && (
            ...
            <p className='flex items-center gap-2 text-slate-800  text-sm'>
             <FaPhoneAlt className='text-green-700' />
              <span className='font-semibold text-black'>Phone Number -</span>{listing.phoneNumber}
            </p>
